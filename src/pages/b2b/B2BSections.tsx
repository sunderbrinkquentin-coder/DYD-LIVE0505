...
function useSectionAnims() {
  ...
  const fadeLeft: Variants = { hidden: { opacity: 0, x: reduce ? 0 : -30 }, show: { opacity: 1, x: 0, transition: { duration: reduce ? 0 : 0.5 } } };
  const fadeRight: Variants = { hidden: { opacity: 0, x: reduce ? 0 : 30 }, show: { opacity: 1, x: 0, transition: { duration: reduce ? 0 : 0.5 } } };
  return { container, fadeUp, fadeLeft, fadeRight, reduce };
}

const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  layers: Layers, brain: Brain, shield: ShieldCheck, flag: Flag,
};
...
export function PlatformOverviewSection() {
  ...
  return (
    <section className="relative py-24 bg-[#0A192F] overflow-hidden border-t border-white/5">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] rounded-full pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(56,189,248,0.08), transparent 70%)', filter: 'blur(80px)' }} aria-hidden="true" />
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#38BDF8]/10 border border-[#38BDF8]/20 mb-4">
            <Sparkles className="w-3.5 h-3.5 text-[#38BDF8]" /><span className="text-xs font-arimo font-bold tracking-wider text-[#38BDF8] uppercase">DYD Standard</span>
          </div>
          <h2 className="font-poppins font-black text-white text-3xl sm:text-4xl lg:text-5xl tracking-tight mb-6">{platformOverview.title}</h2>
          <p className="font-arimo text-lg text-white/70 leading-relaxed">{platformOverview.subtitle}</p>
        </div>
        <motion.div variants={container} initial="hidden" whileInView="show" viewport={VIEWPORT} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {platformOverview.features.map((feature) => {
            const IconComponent = iconMap[feature.icon] || Layers;
            return (
              <motion.div key={feature.title} variants={fadeUp} className="group relative p-8 rounded-2xl bg-white/[0.03] border border-white/10 transition-all duration-300 hover:bg-white/[0.06] hover:border-[#38BDF8]/30 hover:-translate-y-1">
                <div className="w-12 h-12 rounded-xl bg-[#38BDF8]/10 border border-[#38BDF8]/20 flex items-center justify-center mb-6 text-[#38BDF8] group-hover:scale-110 group-hover:bg-[#38BDF8]/20 transition-transform"><IconComponent className="w-6 h-6" /></div>
                <h3 className="font-poppins font-bold text-xl text-white mb-3">{feature.title}</h3>
                <p className="font-arimo text-white/65 text-sm leading-relaxed">{feature.desc}</p>
              </motion.div>
            );
          })}
        </motion.div>
          ...
          {trust.badges.map((badge) => {
            ...
            return (
              <motion.div key={badge} variants={fadeUp} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border-2" style={{ borderColor: 'rgba(56,189,248,0.25)', background: 'rgba(56,189,248,0.04)' }}>
                <Icon className="w-4 h-4 text-[#38BDF8]" aria-hidden="true" /><span className="font-poppins font-bold text-sm text-[#0F1E34]">{badge}</span>
              </motion.div>
            );
          })}
        </motion.div>
...
export function EventsSection() {
  const { events } = b2bContent as typeof b2bContent & {
    events: { title: string; subtitle: string; note: string; entries: Array<{ date: string; title: string; location?: string; audience?: string; topics?: readonly string[]; placeholder?: boolean }> };
  };
      ...
      <div className="max-w-5xl mx-auto">
        <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={VIEWPORT} className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-4 border border-[#DEFF9A]/30 bg-[#DEFF9A]/5">
            <Calendar className="w-3.5 h-3.5 text-[#DEFF9A]" aria-hidden="true" /><span className="font-arimo text-xs font-bold text-[#DEFF9A] uppercase tracking-wide">Live vor Ort</span>
          </div>
              ...
              return (
                <motion.div key={entry.title} variants={fadeLeft} className="relative pl-14 sm:pl-20">
                  ...
                  <div className={`rounded-2xl p-6 border transition-colors ${entry.placeholder ? 'border-dashed border-white/15 bg-white/[0.02]' : 'border-white/10 bg-white/[0.04] hover:bg-white/[0.07]'}`}>
                    <div className="flex flex-wrap items-center gap-3 mb-3"><span className={`font-poppins font-bold text-sm ${accent ? 'text-[#38BDF8]' : 'text-white/40'}`}>{entry.date}</span></div>
                    <h3 className={`font-poppins font-bold text-lg mb-3 ${accent ? 'text-white' : 'text-white/40'}`}>{entry.title}</h3>
                    <div className="flex flex-wrap gap-4">
                      {entry.location && (<div className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-white/40" aria-hidden="true" /><span className="font-arimo text-sm text-white/50">{entry.location}</span></div>)}
                      {entry.audience && (<div className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-white/40" aria-hidden="true" /><span className="font-arimo text-sm text-white/50">{entry.audience}</span></div>)}
                    </div>
                    {entry.topics && entry.topics.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-4">{entry.topics.map((topic) => (<span key={topic} className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-arimo font-semibold" style={{ background: 'rgba(222,255,154,0.10)', color: '#DEFF9A' }}>{topic}</span>))}</div>
                    )}
                  </div>
                </motion.div>
              );
