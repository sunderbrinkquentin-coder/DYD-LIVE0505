  ...
  },
{
    ...
    cta: { label: 'Künstler unterstützen', target: 'unterstuetzen' },
    accent: 'rgba(0,175,190,1)',
    accentAlpha: 'rgba(0,175,190,0.18)',
    glow: 'rgba(0,175,190,0.08)',
  },
  {
    src: '/BobMarley.png',
...
body: JSON.stringify({
            ...
            success_url: ticket.id === 'support'
              ...
              ? `${window.location.origin}/#/festival?shirt_success=1&session_id={CHECKOUT_SESSION_ID}`
...
<div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginBottom: 32, padding: '10px 18px', borderRadius: 999, background: 'linear-gradient(135deg, rgba(200,232,64,0.14), rgba(0,212,212,0.10))', border: '1px solid rgba(200,232,64,0.4)', boxShadow: '0 0 30px rgba(200,232,64,0.12)' }}>
  ...
  <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: 'clamp(14px, 2vw, 17px)', letterSpacing: '0.04em', color: '#eaffb0', lineHeight: 1.2 }}>
    Wahrscheinlich das <span style={{ color: C.lime }}>günstigste Festival</span> in Düsseldorf
  </span>
</div>
            ...
            {(() => {
              ...
              return (
                ...
                <motion.div key={hero.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  transition={{ duration: 0.55 }} className="tr-hero mb-5" style={{ border: '2px solid #00c8c8', boxShadow: '0 0 25px rgba(0,200,200,0.15)' }}>
                    ...
                    {hero.badge && (
                      <div className="flex items-center gap-3 mb-5">
                        <span style={{
                          ...
                          backgroundColor: '#00c8c8', color: '#040c0c',
                          boxShadow: '0 0 10px rgba(0,200,200,0.3)',
                        }}>
                          {hero.badge}
                        </span>
                        <div className="flex items-center gap-1.5">
                          <Star className="w-3 h-3 text-amber-400 animate-pulse" />
                          <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '11px', fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#00c8c8' }}>
...
<div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, padding: '14px 16px', borderRadius: 14, background: 'rgba(0,200,200,0.05)', border: '1px solid rgba(0,200,200,0.16)' }}>
  <div style={{ display: 'flex', alignItems: 'center' }}>
    {['/festival/zirkel-1.webp','/festival/askim-1.webp','/festival/larissa-magnus-1.webp','/festival/justyn-maxx-1.webp','/festival/vio-leen-1.webp'].map((src, i) => (
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12.5, color: 'rgba(180,230,230,0.6)' }}>Stand-Up · Zirkel.WTF · DJs · + mehr</div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-end gap-6 sm:gap-10 mb-7">
                      <div className="flex-1 min-w-0">
                        ...
                        {/* conversion claim */}
                        <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '14px', fontWeight: 800, color: '#00c8c8', marginTop: '12px', lineHeight: 1.5 }}>
                          Das wahrscheinlich günstigste Festival in Düsseldorf.
                        </p>
                      </div>
                      
                      {/* price block (MAX CONVERSION) */}
                      <div className="flex-shrink-0 text-left sm:text-right p-4 rounded-xl bg-black/30 border border-cyan-500/10 min-w-[200px]">
                        {/* Durchgestrichener Marktpreis */}
                        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '11px', color: 'rgba(255,255,255,0.4)', lineHeight: 1.4, marginBottom: '2px' }}>
                          Marktüblich einzeln:<br />
                          <span style={{ textDecoration: 'line-through', fontWeight: 600 }}>
                            Konzert + Stand-Up + DJ = 70,00 €
                          </span>
                        </div>

                        {/* Prozentuale Ersparnis voll im Vordergrund */}
                        <div className="inline-block sm:ml-auto px-2 py-0.5 rounded font-black text-[12px] text-black bg-[#00ffff] shadow-[0_2px_10px_rgba(0,255,255,0.4)] my-1">
                          −{savingsPercent}% ERSPARNIS
                        </div>
                        
                        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '11px', color: '#00ffff', fontWeight: 700, marginBottom: '6px' }}>
                          Du sparst {savingsEuro.toLocaleString('de-DE', { minimumFractionDigits: 2 })} €!
                        </div>

                        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '10px', fontWeight: 600, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(0,175,175,0.4)', marginBottom: '1px' }}>Bundlepreis</div>
                        <span className="price-num" style={{ fontSize: 'clamp(36px, 6vw, 58px)', color: '#00c8c8', lineHeight: 1, fontWeight: 900 }}>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {[
                        { icon: Mic,   label: 'Zirkel.WTF Live (Wert 30€)' },
                        { icon: Laugh, label: 'Stand-Up Comedy (Wert 25€)' },
                        { icon: Disc3, label: 'DJ Night (Wert 15€)' },
                      ].map((item) => (
                        <div key={item.label} className="flex items-center gap-2 px-3.5 py-2 rounded-full"
                          style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)' }}>
                          <item.icon className="w-3.5 h-3.5 flex-shrink-0" style={{ color: '#00c8c8' }} />
                      ...
                      <div className="flex items-center gap-2 px-3.5 py-2 rounded-full"
                        ...
                        <CheckCircle className="w-3.5 h-3.5 flex-shrink-0" style={{ color: '#00c8c8' }} />
                        <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '12px', fontWeight: 600, color: '#00c8c8' }}>Alles in einem Ticket</span>
                      </div>
                    </div>
