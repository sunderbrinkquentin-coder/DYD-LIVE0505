...
const StarRating: React.FC<{ stars: number; total?: number }> = ({ stars, total = 5 }) => (
  <span className="inline-flex items-center gap-px flex-shrink-0">
    {Array.from({ length: total }).map((_, i) => (
      <svg key={i} width="7" height="7" viewBox="0 0 12 12" fill={i < stars ? '#475569' : '#e2e8f0'}>
...
export const ClassicCVTemplate: React.FC<ClassicCVTemplateProps> = ({
  ...
  // 💡 ROBUSTE BULLETPOINT-LOGIK: Findet Bulletpoints IMMER, egal wie die DB sie nennt
  const getBullets = (item: any): string[] => {
    if (!item) return [];
    
    // 1. Suche in Arrays (bulletPoints oder bullet_points)
    ...
    // 2. Fallback: Suche nach Zeilenumbrüchen in der Description
    ...
    return (
      <ul className="space-y-0.5 mt-1" style={{ paddingLeft: 0, listStyle: 'none' }}>
        {bullets.map((text, idx) => {
          // Entfernt überflüssige Bindestriche am Anfang, falls aus der DB kommend
          ...
          return (
            <li key={idx} className="leading-snug" style={{ display: 'flex', alignItems: 'flex-start', gap: '6px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              <span style={{ flexShrink: 0, color: '#475569', fontSize: '14px', lineHeight: '10px', marginTop: '1px', userSelect: 'none' }}>
                •
              </span>
              <span style={{ flex: 1, color: '#334155' }}>{cleanText}</span>
            </li>
          );
        })}
      </ul>
  ...
  const renderExperience = () => {
    ...
    return (
      <div>
        <h2 className="text-[12px] font-bold tracking-[0.15em] uppercase text-slate-800 border-b-2 border-slate-200 pb-1 mb-3">
          Berufserfahrung
        </h2>
            ...
            <div key={idx} data-pdf-section style={{ display: 'block', width: '100%', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              <div className="flex items-baseline justify-between gap-2">
                <EditableText
                  ...
                  className="font-bold text-[13px] text-slate-900 leading-snug"
                ...
                <EditableText
                  ...
                  className="text-[11px] font-medium text-slate-500 text-right min-w-[80px] flex-shrink-0 leading-snug"
              ...
              {item.description && !item.description.includes('\n') && (
                <EditableText
                  ...
                  className="text-[12px] text-slate-600 mt-1.5 leading-snug"
                  multiline
                  placeholder="Beschreibung / Aufgaben"
                />
              )}
              {/* Hier übergeben wir das komplette Item an die Bulletpoint-Funktion! */}
  ...
  const renderEducation = () => {
    ...
    return (
      <div>
        <h2 className="text-[12px] font-bold tracking-[0.15em] uppercase text-slate-800 border-b-2 border-slate-200 pb-1 mb-3 mt-5">
          Ausbildung
        </h2>
        <div className="space-y-4">
          {items.map((item: any, idx: number) => (
            ...
            <div key={idx} className="leading-tight" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              ...
              {item.description && !item.description.includes('\n') && (
                <EditableText
                  ...
                  className="text-[12px] text-slate-600 mt-1.5"
                  multiline
  ...
  const renderListSection = (
    ...
    return (
      <div className="mb-5" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
        <h3 className="text-[11px] font-bold tracking-[0.15em] uppercase text-slate-800 mb-2">
          {label}
        </h3>
        <ul
        ...
        >
          {items.map((item: any, idx: number) => {
            if (options?.showLevelsForLanguages) {
              ...
              return (
                <li key={idx} className="flex justify-between items-center gap-2 text-[11px]">
                  <EditableText
                    value={language}
                  ...
                  {stars > 0 ? (
                    ...
                    <EditableText
                      ...
                      className="text-slate-500 text-right w-16"
                      placeholder="Niveau"
                    />
                  )}
                </li>
            ...
            return (
              <li key={idx} style={{ display: 'inline-flex', marginRight: '5px', marginBottom: '5px', listStyle: 'none' }}>
                <span className="inline-flex items-center px-2.5 py-1 rounded bg-slate-100 border border-slate-200 text-[10px] font-semibold text-slate-700 whitespace-nowrap">
                  <EditableText
                    value={text}
                    onChange={(val) => onUpdateSectionItem(index, idx, 'value', val)}
                    className="text-slate-700 bg-transparent"
                    style={{ width: `${Math.max(20, text.length * 7)}px` }}
                    placeholder="Eintrag"
                  />
                </span>
              </li>
  ...
  return (
    <div
      ...
      style={{
        /* 💡 HIER IST DIE NEUE PROFESSIONELLE SCHRIFTART: Sehr leserlich, modern und elegant */
    ...
    >
      <div className="w-full p-8" style={{ minHeight: '1122px' }}>
        {/* Das Layout ist wieder sicher, ohne die fehlerhaften flex-Erzwingungen */}
        <div className="flex gap-8" style={{ minHeight: '1080px' }}>
          ...
          <aside className="w-1/3 max-w-[33%] pr-6 border-r border-slate-200 flex flex-col">
            <div className="mb-6">
              {photoUrl && (
                <div className="w-28 h-28 rounded-full overflow-hidden border-2 border-slate-200 mb-4 mx-auto shadow-sm">
                  <img
                    src={photoUrl}
                    alt="Profilfoto"
              ...
              <div className="text-center">
                <EditableText
                  ...
                  className="text-[22px] font-extrabold tracking-tight text-slate-900 w-full text-center"
                  placeholder="Dein Name"
                />
                <EditableText
                  value={personalInfo.title}
                  onChange={(val) => onUpdatePersonalInfo('title', val)}
                  className="text-[12px] font-medium text-slate-500 mt-1.5 w-full text-center uppercase tracking-wider"
                        ...
                        <li key={idx} style={{ display: 'inline-flex', marginRight: '5px', marginBottom: '5px', listStyle: 'none' }}>
                          <span className="inline-flex items-center px-2.5 py-1 rounded bg-slate-100 border border-slate-200 text-[10px] font-semibold text-slate-700 whitespace-nowrap">
                            <EditableText
                              ...
                              className="text-slate-700 bg-transparent"
                              style={{ width: `${Math.max(20, rawText.length * 7)}px` }}
                              placeholder="Eintrag"
                            />
                          </span>
                        </li>
            ...
            <div data-pdf-section className="mb-5" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              <section>
                ...
                <EditableText
                  ...
                  className="text-[12px] text-slate-600 leading-relaxed w-full"
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <input
            className="bg-transparent outline-none text-slate-500 font-medium"
