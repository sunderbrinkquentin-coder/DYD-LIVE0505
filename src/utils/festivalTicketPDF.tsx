...
// Einmal rendern — von allen Funktionen unten wiederverwendet
async function buildTicketBlob(ticket: FestivalTicketPDFProps): Promise<Blob> {
  return await pdf(<TicketDocument ticket={ticket} />).toBlob();
}

// Reiner Blob (falls anderswo gebraucht)
...
export async function uploadFestivalTicketPDF(ticket: any, existingBlob?: Blob): Promise<string | null> {
  ...
  if (!ticket?.id || !ticket?.stripe_session_id || !ticket?.ticket_number) {
    console.warn('[ticket-pdf] fehlende Felder, überspringe Upload', ticket);
    return null;
  }

  try {
    const blob = existingBlob ?? await buildTicketBlob(ticket);
    const path = `${ticket.stripe_session_id}/${ticket.ticket_number}.pdf`;

    const { error: upErr } = await supabase.storage
      .from('tickets')
      .upload(path, blob, { contentType: 'application/pdf', upsert: true });
    if (upErr) {
      console.error('[ticket-pdf] Upload fehlgeschlagen:', upErr.message);
      return null;
    }
    ...
    console.error('[ticket-pdf] Fehler:', e?.message || e);
    return null;
  }
}

// Download — nutzt denselben Blob und persistiert die URL nebenbei
export async function downloadFestivalTicketPDF(ticket: any): Promise<void> {
  const blob = await buildTicketBlob(ticket);
  ...
  // Gleichen Blob im Hintergrund hochladen + ticket_pdf_url schreiben
  void uploadFestivalTicketPDF(ticket, blob);
}