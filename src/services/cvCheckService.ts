...
export async function linkCVToUser(
  ...
  try {
    ...
    const { error: atsUpdateError } = await supabase
      .from('ats_analyses')
    ...
    if (atsUpdateError) {
      console.warn('[CV-CHECK] ⚠️ Could not backfill user_id in ats_analyses:', atsUpdateError);
    } else {
      console.log('[CV-CHECK] ✅ Backfilled user_id in ats_analyses');
    }
